...
%l=-0.5*(size(y,1)*(log(det(S))+log(det(K_T))+log(det(K_T+S)))+trace(ym*pinv(S+K_T)*ym'));
l=-0.5*(size(y,1)*(log(det(K_T+S)))+trace(ym*pinv(S+K_T)*ym'));
%log(det(S))
%log(det(K_T))
%log(det(K_T+S))
end
